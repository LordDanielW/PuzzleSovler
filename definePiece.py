...
def segmentSides(piece, debugVis = False, downSampleFactor = 4, cornerTrim = 3):
    ...
    #find peaks
    localPeakindicies = find_peaks_circular(angle_differences, windowSize, minPeakHeight_deg, False)

    #peak candidates
    peakCandidates = [sample_points[i] for i in localPeakindicies if i < len(sample_points)]

    if debugVis:
        draw_gradient_contours(piece, peakCandidates,"Peak Candidates")

    #ordered sets
    cornerSetCandidates = increasing_combinations(len(peakCandidates)-1,4) #sets of 4 ordered indicies from peakCandidates

    bestScore = -1
    bestIndex = -1
    for i, ptIndicies in enumerate(cornerSetCandidates):
        orderedPoints = [peakCandidates[ii] for ii in ptIndicies if ii < len(peakCandidates)]
    ...
    finalCornerIndicies = [localPeakindicies[i] for i in finalIndicies if i < len(localPeakindicies)]
...
def find_peaks_circular(arr, window_width, min_peak_height, include_negative_peaks=False):
    """
...
def score_of_shape(vertices, angleTolerance_deg):
    ...
    for i in range(4):
        ...
        vertex = vertices[i]
        val = type(vertices)
        v1 = (vertices[i][0][0] - vertices[i-1][0][0], vertices[i][0][1] - vertices[i-1][0][1])
        v2 = (vertices[(i+1) % 4][0][0] - vertices[i][0][0], vertices[(i+1) % 4][0][1] - vertices[i][0][1])
    ...
    dist1 = pow(vertices[0][0][0] - vertices[1][0][0],2) + pow(vertices[0][0][1] - vertices[1][0][1],2)
    dist2 = pow(vertices[1][0][0] - vertices[2][0][0],2) + pow(vertices[1][0][1] - vertices[2][0][1],2)
    dist3 = pow(vertices[2][0][0] - vertices[3][0][0],2) + pow(vertices[2][0][1] - vertices[3][0][1],2)
    dist4 = pow(vertices[3][0][0] - vertices[0][0][0],2) + pow(vertices[3][0][1] - vertices[0][0][1],2)
